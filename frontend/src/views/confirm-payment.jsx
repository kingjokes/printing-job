...
export default function ConfirmPayment({
  ...
  const handlePayment = useCallback(
    async (e) => {
      e.preventDefault();
      if (!email)
        return ToastMessage("error", "kindly enter your email to proceed");
      setLoading(true);
      await submitPayment({
        email,
        id: breakdown?.id,
        quotedPrice: quote.totalCost,
      })
      ...
      <DialogContent sx={{ px: 2, pt: 2, pb: 4 }}>
        ...
        <Box sx={{ textAlign: "center" }}>
          ...
          <p style={{ fontSize: 14, lineHeight: 1.8 }}>
            {" "}
            <span>This printing of document would be:</span>
            <br />
            <b style={{ fontSize: 12 }}>
              Total Pages: {formatAmount(breakdown?.totalPages || 0)}
            </b>
            <br />
